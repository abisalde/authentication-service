...
// Uses singleflight to prevent cache stampede - concurrent requests for the same
// username will share a single database query instead of hitting the DB multiple times
func (s *AuthService) CheckUsernameAvailability(ctx context.Context, username string) (bool, error) {
	...
	// Cache miss - use singleflight to ensure only one database query for concurrent requests
	// The key for singleflight ensures all concurrent requests for the same username are deduplicated
